...
const createRide = async ({ user, pickup, destination, vehicleType }) => {
  try {
    ...
    if (!fare) {
      throw new Error("Unable to fetch fare data");
    }
    ...
    const otp = getOtp(6); // Generate OTP with 6 digits
    ...
    const ride = await Ride.create({
      ...
      destination,
      otp,
      fare: fare[vehicleType], // Get the fare for the specific vehicle type
      vehicleType,
      distance,
      duration,
    });
    ...
    throw new Error(
      error.message || "An error occurred while creating the ride"
    );
  }
};
